...
public class ReservasService {
    ...
    @Transactional
    public Long reservarMultiples(List<Long> idEntradas, String sessionId) {
        if (idEntradas == null || idEntradas.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "No se han proporcionado entradas para reservar");
        }
        if (idEntradas.size() > 5) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Solo se pueden reservar hasta 5 entradas a la vez");
        }

        long total = 0;
        for (Long idEntrada : idEntradas) {
            Entrada entrada = this.dao.findById(idEntrada).orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Entrada no encontrada: " + idEntrada));
            if (entrada.getEstado() != Estado.DISPONIBLE) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Entrada no disponible: " + idEntrada);
            }
            Token token = new Token();
            token.setEntrada(entrada);
            token.setSessionId(sessionId);
